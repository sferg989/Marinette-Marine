...
$(document).ready(function() {
    ...
    $("#load_p6_status_labor").click(function() {
        var step        = {};
        var p6Dataval   = $('#text_area_p6_status').val();
        step.code       = code;
        step.action     = "load_p6_status_data";
        step.name       = "Load P6 Status Data";
        step.rpt_period = rpt_period;
        ...
        $("#status_labor").append("<div id = \"img_"+step.action+"\"><br><img src=\"../../inc/images/ajax-loader.gif\" height=\"32\" width=\"32\"/>"+step.name+"<br></div>");
        workers     = new Worker("workers/load_p6_status_data.js");
        workers.onmessage = workerDone;
        workers.postMessage(step);
        function workerDone(e) {
            console.log(e.data.id+" has completed");
    ...
    $("#status_valid_check").click(function() {
        var step = {};
        var data_check;
        step.code       = code;
        step.rpt_period = rpt_period;
        step.action     = "status_valid_check";
        if($("#img_"+step.action.length))
        {
            $("#img_"+step.action).remove();
        }

        step.name   = "Validate Status";
        ...
        workers           = new Worker("workers/status_valid_check.js");
        workers.onmessage = workerDone;
